  ...
  return (
    <div className="bg-gray-100 rounded-md shadow-sm p-6">
      ...
      {features.map((feature, index) => (
        <p key={index} className='mt-3 text-gray-500 flex items-center gap-1'>
          <span className='text-green-700'><Check /></span> {feature}
        </p>
      ))}

      <button 
        onClick={handleSubscription} 
        className="w-full rounded-full mt-4 bg-gradient-to-r from-violet-600 to-fuchsia-500 text-white font-semibold hover:opacity-90 transition py-2 cursor-pointer">
